...
import {IOFT, OFTCoreUpgradeable, MessagingReceipt} from "@layerzerolabs/devtools/packages/oft-evm-upgradeable/contracts/oft/OFTCoreUpgradeable.sol";
...
contract ImplementationOFT is OFTUpgradeable {
    ...
    error ZeroAddress();
    error ZeroAmount();
    error InsufficientBalance(uint userBalance);

    /* ======== EVENTS ======== */

    event TokenCreated(address OFT);
    event TokensAllocated();
    event TokenReceived(bytes32 guid, uint srcEid, uint amount);
    ...
    function sendTokens(
        ...
        address to,
        uint amount
    ) external payable returns (OFTReceipt memory) {
        require(
        ...
        MessagingFee memory fee = OFTCoreUpgradeable(this).quoteSend(
            sendParam,
            false
        );
        ...
        (bytes memory message, bytes memory extraOptions) = this.buildMsg(
            sendParam,
            amountReceivedLD
        );
